...
public class ImplementacjaInterfejsu implements MetodyStrumieniowe {
    ...
    public Figura2D getNajwiekszaFigure() {
        return figury.stream()
                ...
                .max((figura1, figura2) -> Integer.compare(figura1.getWysokosc(), figura2.getWysokosc()))
                .orElse(null);
    }
    ...
    public Figura2D getFigureONajmniejszymPolu() {
        return figury.stream()
                .min((figura1, figura2) -> Double.compare(figura1.polePowierzchni(), figura2.polePowierzchni()))
                .orElse(null);
    }
    ...
    public Figura2D getNajmniejszySzescian() {
        return figury.stream()
                ...
                .min((figura1, figura2) -> Double.compare(((Szescian) figura1).objetosc(), ((Szescian) figura2).objetosc()))
                .orElse(null);
    }
    ...
    public List<Figura2D> getPosortowaneWzgledemPowiezchni() {
        return figury.stream()
                .sorted((figura1, figura2) -> Double.compare(figura1.polePowierzchni(), figura2.polePowierzchni()))
    ...
    public Figura2D getFigureZPosortowanychMalejacoWgObwodu(int nr) {
        return figury.stream()
                .filter(figura -> figura.obwod() != 0)
                .sorted((figura1, figura2) -> Double.compare(figura2.obwod(), figura1.obwod()))
                .skip(nr - 1).findFirst().orElse(null);
    }
    ...
    public List<Figura2D> getPierwszePosortowaneRosnacoWgPowierzchni(int ilosc) {
        return figury.stream()
                ...
                .sorted((figura1, figura2) -> Double.compare(figura1.polePowierzchni(), figura2.polePowierzchni()))
                .limit(ilosc).collect(Collectors.toList());
    }
    ...
    public Map<Integer, Figura2D> mapaWgId() {
        return figury.stream()
                .collect(Collectors.toMap(figura -> figura.getId(), figura -> figura));
    }
